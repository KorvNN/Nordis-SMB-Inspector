...
class WordlistStoreTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.share_path.write_text("# Shares\nPublic\nC$\n", encoding="utf-8")
        self.store = WordlistStore(
    ...
    def test_snapshot_returns_raw_text_and_normalized_unique_counts(self) -> None:
        ...
        self.assertEqual(snapshot[WordlistKind.SHARE].entry_count, 2)
    ...
    def test_save_uses_a_temporary_sibling_and_atomic_replace(self) -> None:
        with patch(
            ...
            self.store.save("share", "Public\nFinance\n")

        temporary, destination = (Path(value) for value in replace.call_args.args)
        self.assertEqual(temporary.parent, self.share_path.parent)
        self.assertEqual(destination, self.share_path)
        self.assertFalse(temporary.exists())

    def test_content_entries_may_be_phrases_and_share_entries_may_not_be_paths(self) -> None:
        document = self.store.save("content", "BEGIN PRIVATE KEY\nşifre burada\n")
        self.assertEqual(document.entry_count, 2)

        for invalid in ("../Finance", "Finance/2026", r"Domain\SYSVOL", ".", ".."):
            with self.subTest(invalid=invalid), self.assertRaisesRegex(
                WordlistStoreError,
                r"^Share wordlist entries must be share names\.$",
            ):
                self.store.save("share", invalid)
    ...
    def test_invalid_utf8_and_storage_errors_have_fixed_safe_messages(self) -> None:
        ...
        store = WordlistStore(content_path=secret_path, share_path=self.share_path)
    ...
    def test_invalid_kind_and_incomplete_path_configuration_are_safe(self) -> None:
        with self.assertRaisesRegex(
            WordlistStoreError,
            r"^Wordlist kind is invalid\.$",
        ):
            self.store.read("DoNotLeakKind")
        with self.assertRaisesRegex(
            WordlistStoreError,
            r"^Both wordlist paths are required\.$",
        ):
            WordlistStore(content_path=self.content_path)
