...
- One scan may be active at a time.
- State, credentials, inventory, findings, and events are process-memory only.

Closing the process discards all scan data. There is no database, result export,
...
limited to 2 MiB, and uploaded ccache data is limited to 1 MiB.
...
- The interface and status labels are currently Turkish even though project
